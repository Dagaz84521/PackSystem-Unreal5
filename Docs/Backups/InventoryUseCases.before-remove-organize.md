...
| 阶段 | ID | 场景 | 涉及的存储 | 状态 |
| --- | --- | --- | --- | --- |
| 第一阶段 | INV-001 | 从格子拿起完整或部分物品 | 玩家背包 -> 鼠标暂存 | 已定义 |
| 第一阶段 | INV-002 | 将鼠标物品放入空格 | 鼠标暂存 -> 玩家背包 | 已定义 |
| 第一阶段 | INV-003 | 将鼠标物品合并到已有堆叠 | 鼠标暂存 -> 玩家背包 | 已定义 |
| 第一阶段 | INV-004 | 将鼠标物品与目标格交换 | 鼠标暂存 <-> 玩家背包 | 已定义 |
| 第一阶段 | INV-005 | 取消并归还鼠标物品 | 鼠标暂存 -> 玩家背包 | 已定义 |
| 第一阶段 | INV-006 | 自动整理物品 | 同一容器 | 已定义 |
| 第二阶段 | INV-007 | 玩家与箱子之间转移物品 | 两个容器 | 待整理 |
| 第三阶段 | INV-008 | 拾取世界物品 | 世界 -> 玩家背包 | 待整理 |
| 第三阶段 | INV-009 | 丢弃物品 | 玩家背包 -> 世界 | 待整理 |
| 第四阶段 | INV-010 | 消耗物品 | 玩家背包 | 待整理 |
| 第四阶段 | INV-011 | 获得奖励物品 | 系统 -> 玩家背包 | 待整理 |
| 第四阶段 | INV-012 | 扣除制作或任务材料 | 玩家背包 | 待整理 |
| 第四阶段 | INV-013 | 装备物品 | 背包 -> 装备槽 | 待整理 |
| 第四阶段 | INV-014 | 更换装备 | 背包 <-> 装备槽 | 待整理 |
| 第四阶段 | INV-015 | 卸下装备 | 装备槽 -> 背包 | 待整理 |
| 第四阶段 | INV-016 | 放下或捡起装有物品的背包 | 世界与背包容器 | 待整理 |
| 第四阶段 | INV-017 | 改变容器容量 | 单个容器 | 待整理 |
| 第四阶段 | INV-018 | 玩家死亡并掉落物品 | 玩家背包 -> 世界 | 待整理 |
| 第四阶段 | INV-019 | 销毁装有物品的容器 | 容器 -> 世界或其他容器 | 待整理 |
| 第四阶段 | INV-020 | 保存与加载物品 | 存档与容器 | 待整理 |
...
下面的记录顺序与总表一致。第一阶段用例已经定义，`INV-008` 保留为后续阶段的填写示例。
...
- 背包组件和格子数据有效。
- Cursor Payload 为空。
- 所有非空 Payload 均满足基本数据约束。

**操作规则**

- 第一阶段只合并相容堆叠，并把空格移到末尾。
- 按当前 Slot 顺序从前向后处理，将后续相容堆叠尽量合并到前面。
- 目标堆叠不能超过单格堆叠上限。
- 合并后保持剩余非空堆叠的相对顺序。
- 当前不按名称、类型、稀有度或重量排序；以后可作为策略扩展。
- 整理不能改变任何物品的总数量。
...
| 失败原因 | 期望结果 |
| --- | --- |
| Cursor Payload 非空 | 不整理，先处理 Cursor |
| 背包或格子数据无效 | 不改变库存 |
...
| 确认 Cursor 为空后请求整理 | 用例接口 | Inventory Interaction Controller |
...
涉及格子查询、提取、定点放置、合并、交换或整理的能力放在这里。
